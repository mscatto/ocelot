...
: QObject(parent), m_id(0), m_summary(summary), m_body(body), m_iconName(iconName), m_timeout(5000), m_autoDelete(true) {
...
bool Notification::show() {
    ...
    = s_notifications->notify(getAppName(), m_id, m_iconName, m_summary, m_body, m_actions, m_hints, m_timeout);
    if(m_id == 0) {
        reply.waitForFinished();
        if(!reply.isValid())
            return false;
...
void Notification::setIconFromByteArray(const QByteArray& arr) {
    setHintByteArray("image_data", arr);
}

/*void Notification::setIconFromPixmap(const QPixmap& pixmap) {
    setHintByteArray("image_data", pixmap.);
}*/
