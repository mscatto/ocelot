...
ClientInstance::ClientInstance (QLocalSocket *socket, QSingleInstancePrivate *parent)
	: QObject (parent), instance (parent), socket (socket), stream (socket) {
...
void ClientInstance::socketError (QLocalSocket::LocalSocketError error) {
	if (error == QLocalSocket::PeerClosedError)
		return;
	qCWarning (logQSingleInstance) << "Failed to receive arguments from client with error:" << socket->errorString();
	deleteLater();
}
