...
void QSingleInstancePrivate::startInstance() {
	...
	if (isMaster) {
		...
		if (!isListening) {
			qCWarning (logQSingleInstance)
				<< "Failed to listen for other instances with error:" << server->errorString();
		} else {
...
void QSingleInstancePrivate::recover (int exitCode) {
	...
	if (recoverAction()) {
		...
		if (res == EXIT_SUCCESS) {
			if (autoClose)
				connect (
					QCoreApplication::instance(), &QCoreApplication::aboutToQuit, q, &QSingleInstance::closeInstance);
		} else if (autoClose)
			q->closeInstance();
	} else
...
void QSingleInstancePrivate::sendArgs() {
	...
	connect (client, &QLocalSocket::connected, this, &QSingleInstancePrivate::clientConnected, Qt::QueuedConnection);
	connect (client, &QLocalSocket::readyRead, this, &QSingleInstancePrivate::sendResultReady, Qt::QueuedConnection);
	connect (client,
			 SIGNAL (error (QLocalSocket::LocalSocketError)),
			 this,
			 SLOT (clientError (QLocalSocket::LocalSocketError)));
...
void QSingleInstancePrivate::clientError (QLocalSocket::LocalSocketError error) {
	qCWarning (logQSingleInstance) << "Failed to transfer arguments to master with error:" << client->errorString();
	recover (error);
}
...
void QSingleInstancePrivate::newArgsReceived (const QStringList &args) {
	if (notifyFn)
		notifyFn();
	QMetaObject::invokeMethod (q, "instanceMessage", Qt::QueuedConnection, Q_ARG (QStringList, args));
}
